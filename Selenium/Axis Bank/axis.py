...
def parse():
    ...
    summary={
        ...
        "accountPattern":details[1].encode('ascii'),
        "balance":balance.encode('ascii'),
        "micrcode":"",
        "iType":"bank",
        "account_type":details[2].encode('ascii'),
        "ifsc_code":details[8].encode('ascii'),
    ...
    with open("919010000738688.csv",'r') as csvfile:
        ...
        for i in datareader:
            ...
            if c>10:
                ...
                date=i[0].split("-")
                if len(((i[3]).strip())):
                    file1.write(str(date[-1]+"-"+date[1]+"-"+date[0]+", "+i[2]+", "+"(-)"+(i[3]).strip()+", "+(i[5]).strip())+"\n")
                else:
                    file1.write(str(date[-1]+"-"+date[1]+"-"+date[0]+", "+i[2]+", "+"(+)"+(i[4]).strip()+", "+(i[5]).strip())+"\n")
    file1.close()
