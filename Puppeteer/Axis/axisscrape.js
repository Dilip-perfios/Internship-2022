...
async function parse() {
    ...
    var account_holder_name = $('label:contains("Account holder")').next().text();
    var account_type = $('label:contains("Account type")').next().text();
    var currency = $('label:contains("Currency")').next().text();
    // fetch the other details mentioned, follow the same for Account summary
    personal_details = {
    ...
    fs.appendFile("output.txt",JSON.stringify(personal_details, null, 2) ,noop)
    // JSON.stringify syntax
    //1)The second param is an optional replacer function which you don't need in this case so null works.
    //2)The third param is the number of spaces to use for indentation.
}
parse()
